...
class User extends CI_Controller
{
    ...
    function dashboard()
    {
        ...
        $start_date = $data['countdown']->result()[0]->start_date;
        $end_date = $data['countdown']->result()[0]->end_date;
         $total_amount = $this->_custom_query("SELECT SUM(amount) as total_amt FROM external_transaction WHERE `date_of_creation` BETWEEN '$start_date' AND '$end_date'");
        $data['txn_between_date'] = $total_amount->result()[0]->total_amt;
...
//        $otp = $this->uri->segment(3);//Generated by Authenticator.
    ...
    function external_transaction(){
        user_auth();
         ...
         $user_id = $_SESSION['etp_user_id'];
        //$id = $_SESSION['etp_user_id'];
        $check_user_kyc = $this->_custom_query("SELECT * FROM kyc WHERE user_id = $user_id");
       
        if($check_user_kyc->result()[0]->status != 2){
            $this->session->set_flashdata('flashdata', 'Your KYC details is not yet verified');
