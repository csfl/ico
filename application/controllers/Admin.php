...
class Admin extends CI_Controller
{
    ...
    function kyc_verified()
    {
        ...
        if($action == 1)
        {
           $this->_custom_query("update kyc set status = 1 where id = $id");
           redirect(base_url('admin/kyc'));
        }
        if($action == 2)
        {
           $this->_custom_query("update kyc set status = 2 where id = $id");
           redirect(base_url('admin/kyc'));
        }else
        {
           redirect(base_url('admin/kyc'));
    ...
    function kyc()
    {
      $data['return'] = $this->_custom_query("select * from kyc where status = 0");
      $data['title'] = 'kyc'; 
